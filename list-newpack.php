...
// $table_data = mysqli_fetch_assoc($conn->query(" SELECT date_created FROM member_newpackage WHERE  userid = 667168  "));  
// $date1= new DateTime($table_data['date_created']);
 

// echo "<pre>";
// print_r($date1);
// echo "</pre>"; 
//     if( $date1->format('Y-m-d') >= $date1->format('Y-m-'.'11') && $date1->format('Y-m-d') <= $date1->format('Y-m-'.'25')  ){
//       //30 date
//      $date1->modify('+1 month');
//     echo  $date1->format('Y-m-'.'30');
//     }else{
//       //15 date
              ...
              <table id="example1" class="table table-bordered table-striped">
                <thead>
                  <tr> 
                      <th>User ID</th>  
                      <th>Fullname</th> 
                      <th class="text-center">Add Capital</th>
                ...
                <tbody> 
                  ...
                  while($row = mysqli_fetch_array($result)) {
                    ...
                    <tr>
                      ...
                      <td class="text-center"> <button type="button" class="btn btn-danger btn-add-capital" value="<?php echo $row['userid']; ?>" data-id="<?php echo $row['fullname']; ?>"  > <span class="glyphicon glyphicon-plus" ></span> Add Capital </button> </td>
                      <td><?php  
                          $sq =  "SELECT  capital  as cap  FROM capital WHERE userid =  ".$row['userid']." GROUP BY date_created" ;  
                          $r = mysqli_query( $conn,$sq  ); 
                          $cap =0;
                          while($capital = mysqli_fetch_assoc( $r )){
                           $cap +=$capital['cap'] ; 
                          }  
                          echo  $cap; 
                      ?></td> 
                     <!--  <td class="text-center"> <button type="button" class="btn btn-success btn-payout" value="<?php echo $row['userid'] ?>"  > <span class="glyphicon glyphicon-briefcase" ></span> Payout </button></td> -->
                      <td class="text-center"><?php 
                          $sqq =  "SELECT sum(qty) as quantity FROM new_purchase WHERE id =  ".$row['userid']." AND payout_stat != 'done' " ;  
                          $s = mysqli_query( $conn,$sqq  ); 
                          $mon_pur = mysqli_fetch_assoc( $s ); 

                          echo number_format($mon_pur['quantity']);
                        ?></td>
                      <!-- <td> -->
                        <?php 
                      // $table_data = $conn->query("SELECT capital, date_created FROM capital WHERE userid = ".$row['userid']." and stat != 'done' ");
                      //       while($vals_data = mysqli_fetch_assoc($table_data) ){ 
                      //       echo "<pre>";
                      //       print_r($t  =+ $vals_data['capital'])   ;
                      //       echo "</pre>"; 
                      //       } ?>
                        
                      <!-- </td>  -->
                      <td class="text-center"><a href="<?php echo $linkpayout ?>" > <button type="button" class="btn btn-primary  " value="<?php echo $row['userid'] ?>"  > <span class="glyphicon glyphicon-shopping-cart" ></span> Payout </button> </a></td> 
                    </tr> 
                  <?php } ?>
                    </tbody>   
              </table>
